...
const SUPABASE_URL = "https://icrqmlnrpwmeqwhviozj.supabase.co";

const SUPABASE_ANON_KEY = "sb_publishable_SjzT8aedhYHN0FTd3CR7cQ_oYd9sfK0";
...
function buildPopup(feature, category) {
  const featureKey = registerFeature(feature, category);
  const props = feature?.properties || {};
 ...
 return `
  <div class="itinerary-popup-card" style="min-width:220px; max-width:280px; text-align:center;">
    <button 
      class="add-itinerary-btn"
      title="Ajouter à mon itinéraire"
      onclick="addFeatureToItinerary('${featureKey.replaceAll("'", "\\'")}')"
    >+</button>
...
async function runSearch() {
  ...
  if (!query) {
   setResultsHtml("<p>Veuillez entrer le nom d’un lieu ou une adresse à Madrid.</p>");
    return;
  }
  ...
  try {
    ...
    temporarySearchMarker.bindPopup(`
  <div class="itinerary-popup-card" style="min-width:220px; max-width:280px; text-align:center;">
    <button 
      class="add-itinerary-btn"
      title="Ajouter à mon itinéraire"
      onclick="addSearchPlaceToItinerary(${lat}, ${lon}, '${escapeHtml(title).replaceAll("'", "\\'")}')"
    >+</button>
...
if (createItineraryBtn) {
  ...
  if (resetMapBtn) {
    resetMapBtn.addEventListener("click", () => {
      ...
      setResultsHtml("<p>Recherchez un lieu à Madrid ou utilisez la géolocalisation.</p>");
    });
  }
...
let itineraryDraft = [];
let savedItineraries = JSON.parse(localStorage.getItem("madridItineraries") || "[]");
let itineraryDisplayLayer = L.layerGroup().addTo(map);

function getFeatureKey(feature, category) {
  const title = getFeatureTitle(feature);
  const latlng = getLatLngFromFeature(feature);
  const lat = latlng ? latlng.lat.toFixed(6) : "0";
  const lng = latlng ? latlng.lng.toFixed(6) : "0";
  return `${category}|${title}|${lat}|${lng}`;
}

function registerFeature(feature, category) {
  const key = getFeatureKey(feature, category);
  featureRegistry.set(key, { feature, category });
  return key;
}
...
window.addFeatureToItinerary = function(key) {
  const item = featureRegistry.get(key);
  if (!item) return;

  if (itineraryDraft.some(el => el.key === key)) {
    showToast("Cet élément est déjà dans votre itinéraire.");
    return;
  }
  ...
  itineraryDraft.push({
    ...
    title: getFeatureTitle(item.feature)
  });

  showToast("Élément ajouté à votre itinéraire.");
};

window.addSearchPlaceToItinerary = function(lat, lng, title) {
  const key = `search|${title}|${lat}|${lng}`;
...
function saveItineraries() {
  localStorage.setItem("madridItineraries", JSON.stringify(savedItineraries));
}

function getCurrentlyVisibleMetroLines() {
  return [
    ...new Set([
      ...visibleMetroLines.keys(),
      ...manualMetroLines.keys()
    ])
  ];
}

async function createItinerary() {
  if (!itineraryDraft.length) {
    showToast("Ajoutez d’abord des éléments à votre itinéraire.");
    return;
  }

  if (!currentUser) {
    showToast("Connectez-vous pour sauvegarder cet itinéraire.");
    return;
  }

  const name = prompt("Nom de votre itinéraire :");
  ...
  const newItinerary = {
    ...
    metro_lines: getCurrentlyVisibleMetroLines()
  };
  ...
  itineraryDraft = [];
  await loadCloudItineraries();
  showToast("Itinéraire sauvegardé.");
}
...
function renderItineraryList() {
  ...
  if (!savedItineraries.length) {
    container.innerHTML = "<p>Aucun itinéraire créé.</p>";
    return;
  }

  container.innerHTML = savedItineraries.map(itinerary => `
    <div class="itinerary-item">
      <label style="flex:1; display:flex; align-items:center; gap:8px;">
        <input type="checkbox" data-itinerary-id="${itinerary.id}">
        <span>${escapeHtml(itinerary.name)}</span>
      </label>

      <button 
        class="delete-itinerary-btn"
        data-delete-id="${itinerary.id}"
        title="Supprimer cet itinéraire"
      >✕</button>
    </div>
  `).join("");

  // Checkbox → afficher itinéraire
  container.querySelectorAll("input[data-itinerary-id]").forEach(input => {
    input.addEventListener("change", displaySelectedItineraries);
  });
...
function hideAllBaseLayersForItinerary() {
  document.querySelectorAll("input[data-layer]").forEach(input => {
    input.checked = false;
    const layerName = input.dataset.layer;
    if (layers[layerName]) map.removeLayer(layers[layerName]);
...
function displaySelectedItineraries() {
  itineraryDisplayLayer.clearLayers();
  hideAllBaseLayersForItinerary();

  const checkedIds = [...document.querySelectorAll("input[data-itinerary-id]:checked")]
    .map(input => input.dataset.itineraryId);
  ...
  checkedIds.forEach(id => {
    ...
    itinerary.items.forEach(item => {
      if (item.type === "feature") {
        const registered = featureRegistry.get(item.key);
        if (!registered) return;

        const layer = L.geoJSON(registered.feature, {
          pointToLayer: (feature, latlng) => {
            return L.marker(latlng, {
              icon: categoryIconMap[registered.category]()
            });
          },
          style: () => ({
            color: getCategoryColor(registered.category),
            weight: 4,
            opacity: 0.9,
            fillColor: getCategoryColor(registered.category),
            fillOpacity: 0.15
          }),
          onEachFeature: (feature, layer) => {
            layer.bindPopup(buildPopup(feature, registered.category));
          }
        });

        itineraryDisplayLayer.addLayer(layer);
      }
      ...
      if (item.type === "search") {
        ...
        itineraryDisplayLayer.addLayer(marker);
      }
    });

    itinerary.metroLines.forEach(lineCode => {
      showManualMetroLine(lineCode);
    });
  });
}
...
async function deleteItinerary(id) {
  ...
  if (!confirm(`Supprimer l’itinéraire "${itinerary.name}" ?`)) return;
...
async function signup() {
  ...
  showToast("Compte créé. Vérifiez votre email si demandé.");
}
...
async function loadCloudItineraries() {
  ...
  if (error) {
    console.error(error);
    showToast("Impossible de charger les itinéraires.");
    return;
  }

  savedItineraries = data.map(row => ({
    id: row.id,
    name: row.name,
    items: row.items || [],
    metroLines: row.metro_lines || []
  }));
